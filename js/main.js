...
// Intersection Observer for animations (only existing elements)
...
// Event tracking
function trackEvent(action,category,label=''){if(typeof gtag!=='undefined'){gtag('event',action,{event_category:category,event_label:label})}}

// Initialize on load
document.addEventListener('DOMContentLoaded',function(){showCookieBanner();document.querySelectorAll('a[href="contact.html"], .cta-primary').forEach(b=>b.addEventListener('click',function(){trackEvent('contact_click','engagement',this.textContent.trim())}));let maxScroll=0;window.addEventListener('scroll',function(){const scrollPercent=Math.round((window.scrollY/(document.body.scrollHeight-window.innerHeight))*100);if(scrollPercent>maxScroll){maxScroll=scrollPercent;if(maxScroll>=50&&maxScroll<75){trackEvent('scroll_50','engagement','50% page viewed')}else if(maxScroll>=100){trackEvent('scroll_100','engagement','Full page viewed')}}});if(typeof initializeLanguage==='function'){initializeLanguage()}});